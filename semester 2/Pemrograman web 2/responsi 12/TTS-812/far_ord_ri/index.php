...
if (isset($_GET['message'])) {
  ...
  include_once("../config.php");
                    ...
                    if (isset($_POST['submit'])) 
                    {
                        ...
                        if ($id_sroom !== '')
                        {
                            $sql = SetConditionAND($sql,"id_sroom LIKE '%$id_sroom%'");
                        }
                        if ($trs_type !== '')
                        {
                            $sql = SetConditionAND($sql,"trs_type LIKE '%$trs_type%'");
                        }
                        if ($no_sp !== '')
                        {
                            $sql = SetConditionAND($sql,"no_sp LIKE '%$no_sp%'");
                        }
                        if ($part_code !== '')
                        {
                            $sql = SetConditionAND($sql,"part_code LIKE '%$part_code%'");
                        }
                    }
                    ...
                    $query = "SELECT * FROM perusahaan " . $sql . " ORDER BY id_sroom ASC";
        ...
        <div class="search-form mb-3">
           <form class="form-inline" action="index.php" method="POST">
              ...
              <div class="form-group mb-2">
                <input type="text" name="id_sroom" value = "<?= ($id_sroom !== '') ? $id_sroom : ''; ?>"  class="form-control" placeholder="Company Name">
              </div>
              <div class="form-group mx-sm-3 mb-2">
                <input type="text" name="trs_type" value = "<?= ($trs_type !== '') ? $trs_type : ''; ?>"class="form-control" placeholder="trs_type">
              </div>
              <div class="form-group mx-sm-3 mb-2">
                <input type="tel" name="no_sp" value = "<?= ($no_sp !== '') ? $no_sp : ''; ?>"class="form-control" placeholder="no_sp">
              </div>
              <div class="form-group mx-sm-3 mb-2">
                <input type="text" name="part_code" value = "<?= ($part_code !== '') ? $part_code : ''; ?>"class="form-control" placeholder="part_code">
              </div>
